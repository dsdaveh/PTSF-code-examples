...
bike_ts <- read_csv(bike_daily_csv) %>%
  ...
  # Create dummy variables using tidyr's pivot_wider
  pivot_wider(names_from = Month, values_from = Month, 
              values_fn = length, values_fill = 0) %>%
  pivot_wider(names_from = DOW, values_from = DOW, 
              values_fn = length, values_fill = 0) %>%
  # Interaction of WorkingDay and Weather
  mutate(across(c(WorkingDay, Weather), ~factor(.x))) %>%
  mutate(WorkingDay_Weather = interaction(WorkingDay, Weather)) %>%
  pivot_wider(names_from = WorkingDay_Weather, values_from = WorkingDay_Weather, 
              values_fn = length, values_fill = 0) |> 
  select(-c(dteday, instant, WorkingDay, Weather)) |> 
  relocate(Date, cnt) |> 
  as_tsibble(index = Date)

# Set up training and validation sets.
# take advantage of case to eliminate variables we don't want
bike_ts_vars <- bike_ts |> 
  select(Date, cnt, 
         matches('^[A-Z]', ignore.case = FALSE))
trn_cutoff_date <- max(bike_ts$Date) - days(90)
bike_trn <- bike_ts_vars |> filter(Date <= trn_cutoff_date )
bike_val <- bike_ts_vars |> filter(Date > trn_cutoff_date)
  
bike_formula_vars <- bike_ts_vars |> 
  select(-c(Date, cnt, Dec, Sat, Working.Rain_Snow)) |> 
  names() |> 
  paste(collapse = " + ") 
bike_formula <- formula(sprintf("cnt ~ trend() + %s", bike_formula_vars))

bike_tslm <- bike_trn |> 
  model(
    tslm = TSLM(bike_formula )
  )
