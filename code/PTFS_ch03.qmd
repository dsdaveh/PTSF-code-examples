...
Since the textbook omits the annotation in it's examples for clarity there is a mismatch between the code and the graphics output. We'll create a simple function t add the annotation to subsequent plots to keep the examples minimal.
...
ridership_annotation <- function(plt, y = 2500) {
  ...
  plt +
    # vertical lines for partions
    geom_vline(xintercept = ymd('2001 Apr 01')) +
    geom_vline(xintercept = ymd('2004 Apr 01')) +
    
    # partition arrows
    annotate("line", 
             x = yearmonth('1997 Jan'), y = y_arrow,
             xend = yearmonth('1999 Apr', yend = y_arrow)) +
    annotate("line", 
             x = ymd('1997 Jan 01'), y = y_arrow,
             xend = ymd('1999 Apr 01', yend = y_arrow)) +
